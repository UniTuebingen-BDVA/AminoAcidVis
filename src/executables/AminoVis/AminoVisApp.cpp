...
AminoVisApp::AminoVisApp(QWidget *parent):QMainWindow(parent), m_settings(configPath()+"/config.ini", QSettings::IniFormat, this){
	setupUi(this);
	...
	connect(actionImport_Layer_Data, &QAction::triggered, this, [this]{
		if(*m_data && m_threads.empty()){
			const QString fileName = QFileDialog::getOpenFileName(this, tr("Import Atom Layer Data"), QFileInfo(m_data->getTitle()).baseName(),tr("Binary Atom Layer Data (*.bald);; Comma Separated Values (*.csv)"));
			if(!fileName.isEmpty()){
				float sasRadius;
				if(m_data->importLayerData(fileName,sasRadius)){
					...
					QMessageBox::warning(this, "Failed to import!","Failed to import atom layer data!\nTarget File: "+fileName,QMessageBox::Ok);
	...
	connect(actionExport_Layer_Data, &QAction::triggered, this, [this]{
		if(*m_data && m_threads.empty()){
			QString fileName = QFileDialog::getSaveFileName(this, tr("Export Atom Layer Data"), QFileInfo(m_data->getTitle()).baseName(), tr("Binary Atom Layer Data (*.bald);; Comma Separated Values (*.csv)"));
			if(!fileName.isEmpty()){
				if(m_data->exportLayerData(fileName, m_currentlyUsedPropeRadius)){
					...
					QMessageBox::warning(this, "Failed to export!","Failed to export atom layer data!\nTarget File: "+fileName,QMessageBox::Ok);
	...
	connect(timelineControlsDockWidget, SIGNAL(visibilityChanged(bool)), actionTimeline_Control_View, SLOT(setChecked(bool)));
	...
	connect(actionReloadQML, &QAction::triggered, this, [this]{;
		...
		timelineQuickWidget->engine()->clearComponentCache();
	 	...
	 	PlainTextMessageBox::qmlErrors(this, "QML compile error!", "'/qml/Timeline.qml' failed to compile with the following message:", timelineQuickWidget->errors());
		...
		filterQuickWidget->engine()->clearComponentCache();
	 	...
	 	PlainTextMessageBox::qmlErrors(this, "QML compile error!", "'/qml/Filters.qml' failed to compile with the following message:", filterQuickWidget->errors());
	...
	{//========= Time edit ========
		...
		//smooth trajectories
		connect(smoothTrajectorysHorizontalSlider, SIGNAL(valueChanged(int)), smoothTrajectorysSpinBox, SLOT(setValue(int)));
		connect(smoothTrajectorysSpinBox, SIGNAL(valueChanged(int)), smoothTrajectorysHorizontalSlider, SLOT(setValue(int)));
	...
	{//========= Timeline View ========
		...
		connect(toolButton_snapshotSettings, &QToolButton::toggled, this, [this](bool b){
			...
			pushButtonTimelineSettings->setText((b)?  ">": "<"); //QString::fromUtf8("\u23F5")QString::fromUtf8("\u23F4")
		});
		...
		connect(widget_timelineScreenshotSettingsArea, SIGNAL(saveHeatmapToClipboard()), this, SLOT(saveHeatmapToClipboard()));
	...
	{//========= Extract Surface View ========
		...
		connect(
			probeSizeDoubleSpinBox,static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged), [this, maxProbeRadius]( double v ) {
			...
			if(m_threads.empty()){ // if there are no threads running then we start
				if(m_data->numberOfFrames() && maxNumberOfThreads > 0 && maxNumberOfThreads < 42){
					...
					for(int start = m_timeline->getStartFrame(); start <= m_timeline->getEndFrame(); start+=step){
						...
						ExtractSurfaceThread* thread = new ExtractSurfaceThread(m_data, start, start+step, probeSizeDoubleSpinBox->value(), this);
						...
						connect(thread, &QThread::finished,
							[=](){
									...
									//top timer if all threads finised
					...
					connect(m_extractSurfaceTimer, &QTimer::timeout,
						[=](){
								...
								if(m_threads.empty()){
									...
									emit updateGlLayers();
									extractSurfaceLayersLabel->setText("Finished in "+QString::number(m_SASTime)+"ms");
									...
									+ "ms Remaining: "+ QString("%1.%2").arg(minutes).arg(seconds, 2, 10, QChar('0'))+"min");
								}
					...
					extractSurfaceLayersLabel->setText("Started with "+QString::number(m_threads.size())+" threads");
				}
...
void AminoVisApp::doOpen(){
	const QString modelFileName = QFileDialog::getOpenFileName(this, tr("Open pdb"), "",tr("Data (*.pdb *.gro );;Protein Database (*.pdb );;Gromos87 (*.gro )"));
	QString path;
...
void  AminoVisApp::doOpen(const QString& pbdFileName, const QString& xtcFileName){
	...
	if(opened){
		...
		//QMessageBox::critical(this,"Failed to open files!","Failed to open files!",QMessageBox::Ok); //TODO better error handling
		actionImport_Layer_Data->setEnabled(false);
		actionExport_Layer_Data->setEnabled(false);
	}
...
void AminoVisApp::setAtomInfo(int id){
	...
	atomInfoTextBrowser->setText(
			"Name: "+ a.name+"\n"
			+"Residue: "+a.residue+"\n"
			+"Position: ("+QString::number(f.positions[id].x)+", "+QString::number(f.positions[id].y)+", "+QString::number(f.positions[id].z)+")\n"
			+"Layer: "+ QString::number(((l.layers.empty())? 0: l.layers[id]))+"\n"
			+"Residue Layer: " + QString::number(m_data->getGroupLayer(a.groupID-1, m_timeline->get(m_timeline->getActiveTracker())->get()  ))+"\n"
...
void AminoVisApp::addGlWidget(int frame){
	...
	RenderDockWidget* GlWidget = new RenderDockWidget(m_settings, m_data, frameObject, m_colors, m_filterAtomsListModel, this);
	...
	if(m_renderes.empty()){
		...
		static_cast<FrameContolWidget*>(listWidgetFrames->itemWidget(listWidgetFrames->item(0)))->pushButton_close->setDisabled(false);
	}
...
void AminoVisApp::removeGlWidget(RenderDockWidget* wd){
	...
	if(m_renderes.size() == 1)
		static_cast<FrameContolWidget*>(listWidgetFrames->itemWidget(listWidgetFrames->item(0)))->pushButton_close->setDisabled(true);
...
QImage AminoVisApp::createHeatmapImage(){
	if(!m_heatmapProvider || m_hss.width <= 0 || m_data->numberOfGroups() <= 0 || m_hss.rowHeight <= 0) return QImage();
	...
	if(totalHeight > 8000){
		QMessageBox::critical(this, "Error", "Heatmap image height is too big!\nTotal height would be "+QString::number(totalHeight)
    ...
    if(m_hss.xLabels){
    	const int count = heatmapWidth/(float)(painter.fontMetrics().width(QString::number(m_timeline->getEnd()))+15);
    	const int step = heatmapWidth/count;
    	for(int i = 0; i < count; i++){
    		painter.drawText(QRect(heatmapXOffset+step*i+3,4, step, xLableHeight)
    						,Qt::AlignLeft | Qt::AlignTop ,
							QString::number(std::floor(m_timeline->getStart() + i * (m_timeline->getEnd()-m_timeline->getStart())/std::round(heatmapWidth/(float)step))));
	...
	for(const paintItem& item: paintItems){
		...
		if(m_hss.yLabels){
			if(item.isResidue){
				...
				painter.drawText(QRect(1,yOffset, yLableWidth, m_hss.rowHeight)
						,Qt::AlignLeft | Qt::AlignVCenter ,QString::number(item.id+1)+":"+groupName);
			}else{
				painter.setFont(atomFont);
				painter.drawText(QRect(1,yOffset, yLableWidth, m_hss.rowHeight)
						,Qt::AlignLeft | Qt::AlignVCenter , ((item.last)? "\u2514 ": "\u251C ")+QString::number(item.id+1)+":"+m_data->getAtom(item.id).name);
				//2514
			}
		}
		painter.drawImage(QRect(heatmapXOffset,yOffset, heatmapWidth, m_hss.rowHeight),
				m_heatmapProvider->requestImage(item.isResidue, item.id, m_colors->p_layersColors, nullptr, {heatmapWidth,m_hss.rowHeight}));
...
void AminoVisApp::saveHeatmapToFile(){
	...
	QString fileName = QFileDialog::getSaveFileName(this, tr("Save as image"), QFileInfo(m_data->getTitle()).baseName()+"_heatmap", tr("Portable Network Graphics (*.png);;  Joint Photographic Experts Group (*.jpg)"));
...
void AminoVisApp::saveHeatmapToHTML(){
	if(m_data->numberOfAtroms() <= 0) return;
	QString fileName = QFileDialog::getSaveFileName(this, tr("Save as HTML"), QFileInfo(m_data->getTitle()).baseName()+"_heatmap", tr("Hypertext Markup Language (*.html)"));
	if(fileName.isEmpty()) return;
	...
	if (data.open(QFile::WriteOnly)) {
	    ...
	    out<<"}],\n";
	    //Nanoseconds
	    out<<"{\"name\": \"layer\", \"title\": \""<<QFileInfo(m_data->getTitle()).baseName()<<"\", \"xaxis\": {\"title\": \"Frames\", \"side\": \"bottom\"}, \"yaxis\": {\"tickfont\": {\"size\": 18}}, \"font\": {\"size\": 32}} );\n";
	    out<<"\t</script>\n";
	    out<<"</body>\n";
	    out<<"</html>";
	}
