...
GLRenderWidget::GLRenderWidget(Atoms *data, Tracker *frame, ColorLibrary *colorLib, FilterAtomsListModel *filter,
...
void GLRenderWidget::initializeGL() {
    ...
    if (!checkGLVersion()) {
        const QMessageBox::StandardButton btn = QMessageBox::critical(this, "GL Version Error",
                                                                      QString("Your Open GL version isn't supported!\nOpenGL version must be at least '4.30'!\nGLSL must also be at least '4.30'!\n"
    ...
    if (!isValid()) {
        if (QMessageBox::critical(this, "Error", QString("Qt failed to init Open GL!"),
    ...
    GLenum err = glewInit();
    if (err != GLEW_OK) {
        if (QMessageBox::critical(this, "Error", QString("Failed to initialize GLEW! Msg: ") +
    ...
    m_FBO = new GL::FrameBufferObject(width(), height(),
                                      {
                                              new GL::RWTexture<glm::vec4>(
                                                      glm::vec4(0, 0, 0, 0)), //output color texture
    ...
    for (const rawcolor &c: m_colorLib->getLayerColors(false))
        colorLayerData.push_back({c.bgra.r / 255.f, c.bgra.g / 255.f, c.bgra.b / 255.f, 0});
    m_colorData = new GL::BufferObject(GL_UNIFORM_BUFFER, colorLayerData.size() * sizeof(glm::vec4),
                                       colorLayerData.data(), GL_STATIC_DRAW);
    checkGLError();
    ...
    */
    ...
    checkGLError();
    m_modelMatrix = glm::rotate(glm::translate(glm::mat4(1), glm::vec3(0, 0, 4)), 90.f, glm::vec3(1, 0, 0));
    m_geometry = new GL::Mesh(GL::Geometry::plane(4.f, 4.f));
    m_axis = new GL::Mesh(GL::Geometry(
            {
                    new GL::GeometryListDataHolder<glm::vec3>(
                            {glm::vec3(0, 0, 0), glm::vec3(1, 0, 0), glm::vec3(0, 0, 0), glm::vec3(0, 1, 0),
                             glm::vec3(0, 0, 0), glm::vec3(0, 0, 1)}),
                    new GL::GeometryListDataHolder<glm::vec3>(
                            {glm::vec3(1, 0, 0), glm::vec3(1, 0, 0), glm::vec3(0, 1, 0), glm::vec3(0, 1, 0),
                             glm::vec3(0.2f, 0.2f, 1), glm::vec3(0.2f, 0.2f, 1)}),
...
void GLRenderWidget::destroyGL() {
    ...
    m_FBO = nullptr;
    if (m_FBO_surface) delete m_FBO_surface;
    m_FBO_surface = nullptr;

    if (m_sh_sfq) delete m_sh_sfq;
    m_sh_sfq = nullptr;
    if (m_sfq) delete m_sfq;
    m_sfq = nullptr;
    ...
    //line
    if (m_lineShader) delete m_lineShader;
    ...
    m_helixShapeShader = nullptr;

    //heatscale
    if (m_heatplane) delete m_heatplane;
    m_heatplane = nullptr;
...
inline void
paintText(QPainter &painter, int x, int y, const QString &text, int fontSize, QFont::Weight weight = QFont::Normal) {
    ...
    {//main scene
        if (m_atoms) {
            ...
            if (m_probeRadius > 0) {
                ...
                (*m_atomShader)("model", glm::mat4(1))("view", m_camera->getViewMatrix())("projection",
                                                                                          m_camera->getPerspectiveMatrix())
                        ("cameraWorldPos", m_camera->getPosition())("zoom", m_camera->getZoom())("scale",
                                                                                                 m_atomMasterScale)
        ...
        if (!m_data->numberOfAtroms()) {
            glEnable(GL_BLEND);
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
            (*m_sh_MVP)("model", m_modelMatrix)("view", m_camera->getViewMatrix())("projection",
                                                                                   m_camera->getPerspectiveMatrix());
            m_sh_MVP->use();
            m_geometry->draw();
        }
        ...
        glLineWidth(1.f);
        m_lineShader->use();
        (*m_lineShader)("model", glm::mat4(1))("view", m_camera->getViewMatrix())("projection",
                                                                                  m_camera->getPerspectiveMatrix());
...
void GLRenderWidget::paintGL() {
    ...
    if (m_drawAxis) {
        ...
        mProj[3][1] = -0.85;
        //mProj[3][2] = -2;
        (*m_lineShader)("model", glm::mat4(1))("view", mMat)("projection", mProj);
        m_axis->draw();
    }
...
*/
    ...
    {//screen filling quad
        ...
        (*m_sh_sfq)("view", m_camera->getViewMatrix())("projection", m_camera->getPerspectiveMatrix())
                ...
                ("blackBorderSize",
                 (m_previewWidth > 0.f) ?
                 (th <= m_FBO->getHeight()) ? (1.f - th / (float) m_FBO->getHeight()) / 2.f :
                 (1.f - ((m_previewWidth / (float) m_previewHeight) * m_FBO->getHeight()) / (float) m_FBO->getWidth()) /
                 2.f
    ...
    if (m_data->getHoveredGroup() > 0 && m_data->getHoveredGroup() <= m_data->numberOfGroups()) {
        if (m_data->getHoveredAtom() >= 0 && m_data->getHoveredAtom() < m_data->numberOfAtroms()) {
            ...
            if (frame.maxLayer > 0)
                paintText(painter, 10, height() - 12, "Layer: " + QString::number(
                        frame.layers[m_data->getHoveredAtom()], 10, 0), 12);
            ...
            if (frame.maxLayer > 0)
                paintText(painter, 10, height() - 12, "Layer: " + QString::number(
                        m_data->getGroupLayer(m_data->getHoveredGroup() - 1, m_frame->get()), 10, 3), 12);
...
void GLRenderWidget::mouseMoveEvent(QMouseEvent *event) {
    ...
    if (event->buttons() & (Qt::LeftButton | Qt::MiddleButton)) {
        ...
        } else if (pickedID != m_data->getHoveredAtom()) {
...
QDebug &operator<<(QDebug &stream, const glm::mat4 &mat) {
    stream << "[" << qSetFieldWidth(9) << mat[0][0] << qSetFieldWidth(0) << ", " << qSetFieldWidth(9) << mat[0][1]
           << qSetFieldWidth(0) << ", " << qSetFieldWidth(9) << mat[0][2] << qSetFieldWidth(0) << ", "
           << qSetFieldWidth(9) << mat[0][3] << qSetFieldWidth(0) << "\n"
           << " " << qSetFieldWidth(9) << mat[1][0] << qSetFieldWidth(0) << ", " << qSetFieldWidth(9) << mat[1][1]
           << qSetFieldWidth(0) << ", " << qSetFieldWidth(9) << mat[1][2] << qSetFieldWidth(0) << ", "
           << qSetFieldWidth(9) << mat[1][3] << qSetFieldWidth(0) << "\n"
           << " " << qSetFieldWidth(9) << mat[2][0] << qSetFieldWidth(0) << ", " << qSetFieldWidth(9) << mat[2][1]
           << qSetFieldWidth(0) << ", " << qSetFieldWidth(9) << mat[2][2] << qSetFieldWidth(0) << ", "
           << qSetFieldWidth(9) << mat[2][3] << qSetFieldWidth(0) << "\n"
           << " " << qSetFieldWidth(9) << mat[3][0] << qSetFieldWidth(0) << ", " << qSetFieldWidth(9) << mat[3][1]
           << qSetFieldWidth(0) << ", " << qSetFieldWidth(9) << mat[3][2] << qSetFieldWidth(0) << ", "
           << qSetFieldWidth(9) << mat[3][3] << qSetFieldWidth(0) << "]";
...
void GLRenderWidget::reloadLayers(int frame) {
    ...
    if (m_styleMode == HelixStyle) {
        ...
        if (m_colorMode == LayerResidueColor) {
            ...
            for (auto itPro = m_data->getProteinStartIDs().begin();
                 itPro < m_data->getProteinStartIDs().end(); itPro++, it++) {
                const unsigned int startGroup = m_data->getAtom((*itPro)).groupID;
                const unsigned int endGroup = (itPro + 1 < m_data->getProteinStartIDs().end()) ? m_data->getAtom(
                        (*(itPro + 1))).groupID : m_data->numberOfGroups();
            ...
            for (auto itPro = m_data->getProteinStartIDs().begin();
                ...
                unsigned int endGroup = (itPro + 1 < m_data->getProteinStartIDs().end()) ? m_data->getAtom(
                        (*(itPro + 1))).groupID : m_data->numberOfGroups();
                QVector<float> avarages;
                avarages.reserve(endGroup - startGroup);
                for (unsigned int i = startGroup; i < endGroup; i++) {
...
void GLRenderWidget::reloadModel() {
    ...
    if (m_data && m_data->numberOfAtroms()) {
        GL::GeometryListDataHolder<glm::vec3> *points = new GL::GeometryListDataHolder<glm::vec3>(
                m_data->numberOfAtroms(), GL_DYNAMIC_DRAW, GL_ARRAY_BUFFER);
        GL::GeometryListDataHolder<glm::vec3> *colors = new GL::GeometryListDataHolder<glm::vec3>(
                m_data->numberOfAtroms(), GL_DYNAMIC_DRAW, GL_ARRAY_BUFFER);
        GL::GeometryListDataHolder<float> *layers = new GL::GeometryListDataHolder<float>(m_data->numberOfAtroms(),
                                                                                          GL_DYNAMIC_DRAW,
                                                                                          GL_ARRAY_BUFFER);
        GL::GeometryListDataHolder<float> *radius = new GL::GeometryListDataHolder<float>(m_data->numberOfAtroms(),
                                                                                          GL_STATIC_DRAW,
                                                                                          GL_ARRAY_BUFFER);
        GL::GeometryListDataHolder<int> *groupID = new GL::GeometryListDataHolder<int>(m_data->numberOfAtroms(),
                                                                                       GL_DYNAMIC_DRAW,
                                                                                       GL_ARRAY_BUFFER);
                ...
                GL::Geometry(
                        {
                                ...
                                new GL::GeometryReferenceDataHolder<int>(m_data->getBonds().data(),
                                                                         m_data->getBonds().size() * 2, GL_STATIC_DRAW,
...
void GLRenderWidget::loadLogoAtoms() {
    m_atoms = new GL::Mesh(
            GL::Geometry(
                    {
                            new GL::GeometryListDataHolder<glm::vec3>(
                                    {glm::vec3(0, 0, 0), glm::vec3(1, 1, 0), glm::vec3(1, -1, 1),
                                     glm::vec3(-1, -1, -1)}), // points
                            new GL::GeometryListDataHolder<glm::vec3>(
            ...
            GL::Geometry(
                    {
                            ...
                            new GL::GeometryBufferReferenceHolder<float>(m_atoms->VBO[3]), // radius
                            new GL::GeometryBufferReferenceHolder<int>(m_atoms->VBO[4]), // groupID
                            new GL::GeometryListDataHolder<int>({0, 1, 0, 2, 0, 3, 0, 4}, GL_STATIC_DRAW,
                                                                GL_ELEMENT_ARRAY_BUFFER), //index
                    }
            ), GL_LINES);
}
...
void getHelixBuffers(QVector<glm::vec3> &points, QVector<helixData> &data) {
    ...
    for (QVector<glm::vec3>::const_iterator it = points.begin();
         it < points.end() - spline.getDegree(); it += spline.getDegree() - 1)
    ...
    for (auto itDir = directions.begin(); itDir < directions.end(); itDir++, itData++, itPos++) {
        ...
        itData->capNormal = (nextDir != directions.end()) ? glm::vec4((*itDir + (*nextDir)) / 2.f, 0) : glm::vec4(
                *itDir, 0);
...
void GLRenderWidget::reloadHelix() {
    ...
    for (auto itPoints = m_data->getAtoms().begin(); itPoints < m_data->getAtoms().end(); itPoints++) {
        ...
        if (itPoints->proteinID != currentProteinID) {
            //create helix
            ...
            if (data.size() > 1) {
                ...
                m_helix_data.push_back(
                        new GL::BufferObject(GL_UNIFORM_BUFFER, data.size() * sizeof(helixData), data.data(),
                                             GL_DYNAMIC_DRAW));
                m_helix_color.push_back(
                        new GL::BufferObject(GL_UNIFORM_BUFFER, residueCount * sizeof(float), NULL, GL_DYNAMIC_DRAW));
                m_helix.push_back(new GL::Mesh(GL::Geometry::flatBonedCylinder(8, data.size() - 1)));
            }

            points.clear();
            currentProteinID = itPoints->proteinID;
            residueCount = 1;
        }
        if (itPoints->name == "CA") {
            if (m_data->numberOfFrames()) {
    ...
    if (!points.empty()) {
        ...
        if (data.size() > 1) {
            qDebug() << "Created helix spline with" << data.size() << "points.";
            m_helix_data.push_back(new GL::BufferObject(GL_UNIFORM_BUFFER, data.size() * sizeof(helixData), data.data(),
                                                        GL_DYNAMIC_DRAW));
            m_helix_color.push_back(
                    new GL::BufferObject(GL_UNIFORM_BUFFER, residueCount * sizeof(float), NULL, GL_DYNAMIC_DRAW));
            m_helix.push_back(new GL::Mesh(GL::Geometry::flatBonedCylinder(8, data.size() - 1)));
...
void GLRenderWidget::renderBall() {
    ...
    (*m_atomShader)("model", glm::mat4(1))("view", m_camera->getViewMatrix())("projection",
            ...
            "propeRadius", 0.f)
            ("hideWater", !m_waterVisible)("onlyBackBone", false)("waterSkip", m_waterSkip)("applyFilters",
                                                                                            m_filtersEnabled)
    ...
    if (m_data->numberOfAtroms())
        (*m_atomShader)("hoveredAtom", m_data->getHoveredAtom())("hoveredGroup", m_data->getHoveredGroup());
    else (*m_atomShader)("hoveredAtom", -1)("hoveredGroup", -1);
    m_atoms->draw();
}

void GLRenderWidget::renderBallStik() {
    (*m_atomShader)("model", glm::mat4(1))("view", m_camera->getViewMatrix())
    ...
    if (m_bonds) {
        (*m_bondShader)("model", glm::mat4(1))("view", m_camera->getViewMatrix())("projection",
                                                                                  m_camera->getPerspectiveMatrix())
        ...
        if (m_data->numberOfAtroms())
            (*m_bondShader)("hoveredAtom", m_data->getHoveredAtom())("hoveredGroup", m_data->getHoveredGroup());
...
void GLRenderWidget::renderBackbone() {
    (*m_atomShader)("model", glm::mat4(1))("view", m_camera->getViewMatrix())
            ...
            ("selectedAtom", m_data->getSelectedAtom())("equalSize", true)("colorMode", (int) m_colorMode)(
            "propeRadius", 0.f)
            ("hideWater", !m_waterVisible)("onlyBackBone", true)("waterSkip", m_waterSkip)("applyFilters",
                                                                                           m_filtersEnabled)
            ("colors", *m_colorData);
...
void GLRenderWidget::renderHelix() {
    if (!m_helix.empty()) {
        m_helixShapeShader->use();
        (*m_helixShapeShader)("model", glm::mat4(1))("view", m_camera->getViewMatrix())("projection",
                                                                                        m_camera->getPerspectiveMatrix())
        ...
        for (GL::Mesh *m: m_helix) {
            (*m_helixShapeShader)("data0", *(*itData0))("data1", *(*itData1))("colors", *m_colorData)(
                    "aminoAcidGroupOffset", m_data->getAtom(*itProteinStart).groupID - 1);
            m->draw();
            itData0++;
            itData1++;
            itProteinStart++;
...
void GLRenderWidget::renderHeatscale(int width, int height, QPaintDevice *device) {
    if (!m_data->numberOfAtroms() || !m_data->numberOfFrames() || m_data->getLayers().size() <= m_frame->get() ||
        m_colorLib->p_layersColors.size() < 2)
        return;
    int maxLayer = m_data->getLayer(m_frame->get()).maxLayer - 1;
    if (maxLayer <= 0 || maxLayer >= m_colorLib->p_layersColors.size())
        maxLayer = m_colorLib->p_layersColors.size() - 1;
    ...
    for (const Atoms::atom &a: m_data->getAtoms()) {
        ...
        if (currentResidue != int(a.groupID)) {
            ...
            if (!m_filter->getFilterResidueResults().isEmpty()) {
                for (const ResultFilterItem &v: m_filter->getFilterResidueResults().value(a.groupID - 1,
        ...
        if (hideResidue) flag |= FLAG_IS_FILTER_HIDDEN;
            ...
            if (!m_filter->getFilterAtomResults().isEmpty()) {
                for (const ResultFilterItem &v: m_filter->getFilterAtomResults().value(index,
                                                                                       QList<ResultFilterItem>())) {
                    if (v.item->renderViewEnabled && v.type() == QVariant::Bool && v.toBool()) {
