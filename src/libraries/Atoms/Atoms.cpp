			...
			if(token == QXmlStreamReader::TokenType::StartElement){
				...
				if(name == "Residue"){ // Residue Tag
					...
					if(residueName.isEmpty()){
						QMessageBox::critical(nullptr,
						"XML Parse Error","File: "+file+"\nError: Residue name not defined at "+QString::number(xml.lineNumber())+"!" ,
						QMessageBox::Ok);
						return;
					}
					//store data
					...
					if(residueName.isEmpty()){
						QMessageBox::critical(nullptr,
						"XML Parse Error","File: "+file+"\nError: Residue must have a 'name' attribute, at  "+QString::number(xml.lineNumber())+"!" ,
						QMessageBox::Ok);
						return;
					}
					...
					if(name.isEmpty()){
						QMessageBox::critical(nullptr,
						"XML Parse Error","File: "+file+"\nError: Atom tag must have a 'name' attribute, at "+QString::number(xml.lineNumber())+"!" ,
						QMessageBox::Ok);
						return;
					}
		...
		while (!xml.atEnd()) {
			...
			if(token == QXmlStreamReader::TokenType::StartElement){
				...
				if(name == "Residue"){ // Residue Tag
					...
					if(residueName.isEmpty()){
						QMessageBox::critical(nullptr,
						"XML Parse Error","File: "+file+"\nError: 'Bond' tag must be inside 'Residue' tag at  "+QString::number(xml.lineNumber())+"!" ,
						QMessageBox::Ok);
						return;
					}
					...
					if(from.isEmpty() || from.isEmpty()){
						QMessageBox::critical(nullptr,
						"XML Parse Error","File: "+file+"\nError: Bond tag must have a 'from' and 'to' attribute, at "+QString::number(xml.lineNumber())+"!" ,
						QMessageBox::Ok);
						return;
					}
			...
			if(token == QXmlStreamReader::TokenType::StartElement){
				...
				if(name == "Residue"){ // Residue Tag
					...
					if(shortName.isEmpty() || name.isEmpty()){
						QMessageBox::critical(nullptr,
						"XML Parse Error","File: "+file+"\nError: Residue tag must have a 'short' and 'name' attribute, at "+QString::number(xml.lineNumber())+"!" ,
						QMessageBox::Ok);
						return;
					}
			...
			while (!xml.atEnd()) {
				...
				if(token == QXmlStreamReader::TokenType::StartElement){
					...
					if(name == "Element"){ // Residue Tag
						...
						if(name.isEmpty()){
							QMessageBox::critical(nullptr,
							"XML Parse Error","File: "+file+"\nError: Element tag must have a 'name' attribute, at "+QString::number(xml.lineNumber())+"!" ,
							QMessageBox::Ok);
							return;
						}
	...
	while (!in.atEnd()){
	  ...
	  if(line.startsWith("ATOM")){
		  ...
		  if(parameters.size() != 11 && parameters.size() != 12 ){
			  PDB_ERROR("NOT enough parameters inside a ATOM tag! (There should be 12 or 11 but there are "<<parameters.size()<<")");
			  clear();
			  file.close();
			  QAbstractItemModel::endResetModel();
			  return false;
		  ...
		  QPair<QColor,float> atomInfo = m_elemetColorsAndVdW.value(parameters[parameters.size()-1], {QColor("#d985f5"), 1} );
		  ...
		  if(currentGroup != (int)m_model.last().groupID){
			  ...
			  if(m_model.last().residue == "SOL" || m_model.last().residue == "HOH" || m_model.last().residue.toLower() == "water") m_waterCount++;
		  }
...
bool Atoms::openGRO(const QString& path){
	...
	while (line_number < size && !in.atEnd()){
	  ...
	  const QStringList parameters = {line.mid(0,5).trimmed(), line.mid(5,5).trimmed(), line.mid(10,5).trimmed(), line.mid(15,5).trimmed() ,
			  line.mid(20,8).trimmed(),line.mid(28,8).trimmed(),line.mid(36,8).trimmed()};
	  ...
	  m_model.push_back(
			  {
				  ...
				  glm::vec3(parameters[4].toFloat()*10.f, parameters[5].toFloat()*10.f,parameters[6].toFloat()*10.f),
				  atomInfo.first, atomInfo.second
			  });
	  ...
	  if(groupID != currentGroupID){
		  ...
		  if(m_model.last().residue == "SOL" ||m_model.last().residue == "HOH" || m_model.last().residue.toLower() == "water") {
			  m_waterCount++;
		  }
	  }
	}
...
bool Atoms::openXTC(const QString& path){
	...
	if(numberOfAtroms != m_model.size()){
		qDebug()<<"[ERROR]: The number of atoms inside the XTC isn't the same as in the model. ("<<numberOfAtroms<<" != "<<m_model.size()<<")"<<path;
		QAbstractItemModel::endResetModel();
		clear();
		return false;
	}
...
bool Atoms::simulateXTC(){
	...
	frame.positions.reserve(m_model.size());
	frame.time = 0;
	for(const Atoms::atom& a: m_model){
		frame.positions.push_back(a.position);

		if(a.position.x < frame.box.min.x) frame.box.min.x = a.position.x;
		if(a.position.y < frame.box.min.y) frame.box.min.y = a.position.y;
		if(a.position.z < frame.box.min.z) frame.box.min.z = a.position.z;

		if(a.position.x > frame.box.max.x) frame.box.max.x = a.position.x;
		if(a.position.y > frame.box.max.y) frame.box.max.y = a.position.y;
		if(a.position.z > frame.box.max.z) frame.box.max.z = a.position.z;
	}
	...
	if(info.suffix() == "bald"){
		...
		if ( file.open(QIODevice::ReadOnly) ){
			...
			if(numAtoms != m_model.size() || numFrames != m_layers.size() ){
				qDebug()<<"[importLayerData:"<<__LINE__<<"]: Number of atoms or frames is not equal to the loaded data!"<<"("<<numAtoms<<"!="<<m_model.size()<<" || "<<numFrames<<"!="<<m_layers.size()<<")";
				return false;
			}
...
float Atoms::getAtomLayer(int atomIndex, int frame, bool applyFilters, bool isTimeline) const{
	...
	float value = 0;
	bool found = false;
	if( !(isTimeline && !m_filterAtomsListModel->isEnabled()) && !m_filterAtomsListModel->getFilterResidueResults().isEmpty() && applyFilters){
			for( const ResultFilterItem& v : m_filterAtomsListModel->getFilterAtomResults().value( atomIndex, QList<ResultFilterItem>() )){
				...
				if(v.item->isLive){
					...
					QVariant result = v.item->node->call(true, atomIndex, getAtomName(atomIndex), const_cast<Atoms*>(this), const_cast<Atoms*>(this), m_timeline, vars);
					//qDebug()<<" RESULT "<<result<<result.type();
					if(result.type() == QVariant::Int){ value += result.toInt(); found = true;}
					else if(result.type() == QVariant::Double){ value += result.toFloat(); found = true;}
				}else{
					if(v.type() == QVariant::Int){ value += v.toInt(); found = true;}
					else if(v.type() == QVariant::Double){ value += v.toFloat(); found = true;}
				}
			}
		}
...
float Atoms::getGroupLayer(int groupIndex, int frame, bool applyFilters, bool isTimeline) const{
	...
	bool found = false;
	if( !(isTimeline && !m_filterAtomsListModel->isEnabled()) && !m_filterAtomsListModel->getFilterResidueResults().isEmpty() && applyFilters){
			for( const ResultFilterItem& v : m_filterAtomsListModel->getFilterResidueResults().value( groupIndex, QList<ResultFilterItem>() )){
				if(isTimeline && !v.item->timelineViewEnabled) continue;
				if(!isTimeline && !v.item->renderViewEnabled) continue;
				if(v.item->isLive){
					Atom::Variables vars;
					vars["frame"] = Atom::FilterSharedPtr(new Atom::Integer(frame));
					QVariant result = v.item->node->call(false, groupIndex, getGroupName(groupIndex), const_cast<Atoms*>(this), const_cast<Atoms*>(this), m_timeline, vars);
					//qDebug()<<" RESULT "<<result<<result.type();
					if(result.type() == QVariant::Int){ value += result.toInt(); found = true;}
					else if(result.type() == QVariant::Double){ value += result.toFloat(); found = true;}
				}else{
...
float Atoms::getGroupLayerAvarage(int groupIndex, const Atoms::layerFrame& frame) const{
	QVector<float>::const_iterator end = frame.layers.begin() + ((groupIndex == m_groupStartIDs.size()-1)? m_model.size() : m_groupStartIDs[groupIndex+1]);
...
void Atoms::fillGroupLayerAvarage(QVector<float>& avarages, const Atoms::layerFrame& frame) const{
	...
	for(int groupIndex = 0; groupIndex < m_groupStartIDs.size(); groupIndex++){
		QVector<float>::iterator end = avarages.begin() + ((groupIndex == m_groupStartIDs.size()-1)? m_model.size() : m_groupStartIDs[groupIndex+1]);
...
QVector<Atoms::atom>::iterator Atoms::findAtomInGroup(int groupIndex, const QString& atomName){
	...
	QVector<Atoms::atom>::iterator end = m_model.begin() + ((groupIndex == m_groupStartIDs.size()-1)? m_model.size() : m_groupStartIDs[groupIndex+1]);
...
QVector<Atoms::atom>::const_iterator Atoms::findAtomInGroup(int groupIndex, const QString& atomName) const{
	if(groupIndex < 0 || groupIndex >= m_groupStartIDs.size()) return m_model.end();
	QVector<Atoms::atom>::const_iterator end = m_model.begin() + ((groupIndex == m_groupStartIDs.size()-1)? m_model.size() : m_groupStartIDs[groupIndex+1]);
...
bool Atoms::createBond(int fromResidue, const QString& fromAtomName, int toResidue, const QString& toAtomName){
	...
	if(itFrom->protainID != itTo->protainID) return false;
    ...
    if(index.parent().isValid()){
    	switch (role) {
			case  Qt::DisplayRole:{
				...
				return QVariant::fromValue(getAtomName(atomIndex )); }break; //the display text
			case  TypeRole:{return QVariant::fromValue(QString("atom"));}break;
			case  isAtomRole:{return QVariant::fromValue(true);}break;
			case  ItemIndexRole:{return QVariant::fromValue(index);}break;
			case  ItemRowRole:{
				int atomIndex = getGroupStartID(index.parent().row())+index.row();
				return QVariant::fromValue(atomIndex ); }break;
		}
    }else{
    	switch (role) {
			case  Qt::DisplayRole:{return QVariant::fromValue(getGroupName(index.row())); }break; //the display text
			case  TypeRole:{return QVariant::fromValue(QString("residue"));}break;
			case  isAtomRole:{return QVariant::fromValue(false);}break;
			case  ItemIndexRole:{return QVariant::fromValue(index);}break;
			case  ItemRowRole:{return QVariant::fromValue(index.row());}break;
		}
    }
...
QDebug operator<< (QDebug d, const Atoms::atom &m) {
    d << "Atom[typeName: "<<m.name<<" atomName: "<<m.element<<" groupName: "<<m.residue<<" groupID: "<<m.groupID<<" position: "<<m.position<<"]";
    return d;
}

QDebug operator<< (QDebug d, const TrajectoryStream::xtcFrame &m) {
    d << "Frame[time: "<<m.time<<" precision: "<<m.precision<<" size: "<<m.positions.size()<<" i: "<<m.index<<"]";
    return d;
}
